...
const AlertsDashboardScreen: React.FC = () => {
          ...
          <View style={styles.alertLeft}>
            ...
            <View style={styles.alertInfo}>
              <Text style={styles.alertTitle}>
                {alert.type === 'warning' ? 'Budget Warning' :
  ...
  const renderSuggestionItem = (suggestion: SmartSuggestion, index: number) => {
    ...
    return (
      ...
      >
        <View style={styles.alertContent}>
          <View style={styles.alertLeft}>
            <View style={[styles.alertIcon, { backgroundColor: suggestionColor + '20' }]}>
              <Ionicons name={suggestionIcon as any} size={20} color={suggestionColor} />
            </View>
            <View style={styles.alertInfo}>
              <Text style={styles.alertTitle}>{suggestion.title}</Text>
              <Text style={styles.alertCategory}>{suggestion.categoryName}</Text>
              <Text style={styles.alertMessage}>{suggestion.message}</Text>
            </View>
          </View>
          <View style={styles.alertRight}>
            <Text style={styles.alertTime}>{formatDate(suggestion.createdAt)}</Text>
            <TouchableOpacity
              style={styles.dismissButton}
  ...
  const renderEmptyState = () => (
    <Animated.View entering={FadeIn.delay(200)} style={styles.emptyState}>
      <Ionicons 
        name={activeTab === 'alerts' ? 'notifications-off' : 
              activeTab === 'history' ? 'time-outline' : 'bulb-outline'} 
        size={64} 
        color={colors.textSecondary} 
      />
      <Text style={styles.emptyTitle}>
        {activeTab === 'alerts' ? 'No Active Alerts' :
         activeTab === 'history' ? 'No Alert History' :
         'No Smart Suggestions'}
      </Text>
      <Text style={styles.emptyDescription}>
        {activeTab === 'alerts' ? 'You\'re all caught up! No budget alerts at the moment.' :
         activeTab === 'history' ? 'Your budget alert history will appear here.' :
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View entering={SlideInRight.delay(300)} style={styles.headerActions}>
          <TouchableOpacity
            style={styles.headerButton}
            onPress={() => navigation.navigate('AlertSettings' as any)}
          >
      ...
      <Animated.View entering={FadeInDown.delay(200)} style={styles.tabContainer}>
        ...
        >
          <Ionicons 
            name="bulb" 
            size={16} 
            color={activeTab === 'suggestions' ? colors.white : colors.textSecondary} 
          />
