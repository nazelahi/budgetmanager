...
const SettingsScreen: React.FC = () => {
  ...
  const { data, updateSettings, exportToJSON, exportToCSV, refreshData } =
    useApp();
  ...
  const handleThemeToggle = async (theme: "light" | "dark") => {
    try {
      ...
      ToastService.error("Error", "Failed to update theme");
    }
  };
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        try {
          ...
          ToastService.error("Error", "Invalid JSON file format");
          return;
        }
        if (!importedData || typeof importedData !== "object") {
          ToastService.error("Error", "Invalid data format");
          return;
        }
        // Basic schema checks to prevent corruption
        const missingTopLevel = [
          "transactions",
          "categories",
          "settings",
        ].filter((k) => !(k in importedData));
        if (missingTopLevel.length > 0) {
          ToastService.error(
            "Error",
            `Missing keys: ${missingTopLevel.join(", ")}`,
          );
          return;
        }
        // Defensive validation of arrays
        if (
          !Array.isArray(importedData.transactions) ||
          !Array.isArray(importedData.categories)
        ) {
          ToastService.error("Error", "Invalid list structures in import");
          return;
        }
        // Sanitize suspicious fields
          ...
          .map((t: any) => ({
            ...
            description:
              typeof t.description === "string"
                ? t.description.slice(0, 256)
                : "",
            type: t.type === "income" || t.type === "expense" ? t.type : "expense",
            date: typeof t.date === "string" ? t.date : new Date().toISOString().split("T")[0],
          }));
        importedData.categories = importedData.categories
          .filter((c: any) => c && typeof c === "object")
          .map((c: any) => ({
            ...c,
            name: typeof c.name === "string" ? c.name.slice(0, 50) : "Category",
        ...
        await refreshData();
        ToastService.success("Success", "Data imported successfully");
      }
    } catch (error) {
  ...
  const handleClearData = async () => {
    try {
      ...
      ToastService.success(
        "Success",
        "All data has been cleared. Redirecting to setup...",
      );
      (navigation as any).navigate("Setup");
    } catch (error) {
      ToastService.error("Error", "Failed to clear data");
    }
  };
  ...
  const handleExportData = async () => {
    try {
      ...
      ToastService.success(
        "Export Complete",
        `Full backup exported. Size: ${(jsonData.length / 1024).toFixed(1)} KB`,
      );
    } catch (error) {
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <SettingItem
            ...
            title="Currency"
            subtitle={
              currencyOptions.find((c) => c.code === data.settings.currency)
                ?.name
            }
          ...
          <SettingItem
            ...
            rightComponent={
              <Switch
                ...
                thumbColor={
                  data.settings.notifications ? colors.white : colors.gray500
                }
              />
            }
          />
          ...
          <SettingItem
            ...
            rightComponent={
              <Switch
                ...
                onValueChange={(value) =>
                  handleThemeToggle(value ? "dark" : "light")
                }
                trackColor={{ false: colors.gray300, true: colors.primary }}
                thumbColor={
                  data.settings.theme === "dark" ? colors.white : colors.gray500
                }
              />
            }
          />
        ...
        <View style={styles.section}>
          ...
          <SettingItem
            ...
            onPress={() =>
              ToastService.info("Help", "Contact support@budgetmanager.com")
            }
          />
