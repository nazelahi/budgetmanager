import { Budget, BudgetStats, BudgetAlert, Transaction, Category, AlertHistory, SmartSuggestion, AlertSettings } from '../types';
...
class BudgetService {
   ...
   */
  async getBudgetStats(period: 'monthly' | 'yearly' = 'monthly'): Promise<BudgetStats> {
    try {
      const data = await StorageService.getData();
      const budgets = (data.budgets || []).filter(b => b.isActive && b.period === period);
      ...
      const categoryStats = budgets.map(budget => {
        const categoryTransactions = periodTransactions.filter(t => t.category === budget.categoryName);
  ...
  async checkBudgetAlerts(): Promise<void> {
    try {
      ...
      const alertSettings = data.alertSettings || await StorageService.getAlertSettings();
      ...
      for (const category of stats.categories) {
        ...
        for (const threshold of alertSettings.warningThresholds) {
          if (category.percentageUsed >= threshold && category.percentageUsed < 100) {
            const hasWarning = existingAlerts.some(a => 
            ...
            if (!hasWarning) {
              ...
              if (alertSettings.smartSuggestions) {
                await this.generateSmartSuggestions(budget, category, 'warning');
              }
            }
          }
        }
        ...
        if (category.isOverBudget) {
          const hasExceeded = existingAlerts.some(a => 
            a.budgetId === budget.id && 
            a.type === 'exceeded' && 
            !a.isRead &&
            new Date(a.createdAt) > new Date(Date.now() - 24 * 60 * 60 * 1000) // Within last 24 hours
        ...
        if (category.percentageUsed >= 50 && category.percentageUsed < 80 && category.remaining > 0) {
          const hasAchieved = existingAlerts.some(a => 
            a.budgetId === budget.id && 
            a.type === 'achieved' && 
            !a.isRead &&
            new Date(a.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // Within last 7 days
          );
   ...
   */
  async getBudgetRecommendations(): Promise<Array<{ categoryName: string; recommendedAmount: number; reason: string }>> {
    try {
      const data = await StorageService.getData();
      const transactions = data.transactions || [];
   ...
   */
  async getBudgetPerformance(months: number = 6): Promise<Array<{
    month: string;
    budgeted: number;
    spent: number;
    remaining: number;
    ...
    try {
      const data = await StorageService.getData();
      const budgets = (data.budgets || []).filter(b => b.isActive && b.period === 'monthly');
      const transactions = data.transactions || [];
  ...
  async getBudgetUtilization(): Promise<{
    ...
    try {
      ...
      return {
        ...
        utilizationPercentage: expenseCategories.length > 0 
          ? (activeBudgets.length / expenseCategories.length) * 100 
          : 0,
        overBudgetCategories: stats.categories.filter(c => c.isOverBudget).length,
      };
    } catch (error) {
   ...
   */
  async generateSmartSuggestions(budget: Budget, category: any, alertType: 'warning' | 'exceeded'): Promise<void> {
    try {
      const data = await StorageService.getData();
      const transactions = data.transactions || [];
      ...
      const recentTransactions = transactions
        .filter(t => t.category === category.categoryName && t.type === 'expense')
      ...
      for (const suggestion of suggestions) {
        const exists = existingSuggestions.some(s => 
          s.budgetId === suggestion.budgetId && 
          s.type === suggestion.type &&
          new Date(s.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // Within last 7 days
        );
